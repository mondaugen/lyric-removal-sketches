...
# arg 3 is the number of divisions of the height from which to start searches
...
numdivs     = float(sys.argv[3])
numsearches = float(sys.argv[4])
negbound    = float(sys.argv[5])
posbound    = float(sys.argv[6])

# calculate all the starting y-positions
y_starts = [img.ul.y + float(img.height) / numdivs * ( float(i) + 0.5 ) for i \
    in xrange(int(numdivs))]
...
for ys in y_starts:
  ...
  black_counts = [ count_colour_under_function(img, \
      linear_equation_from_points((img.ul.x, ys), (img.ur.x, ye))) \
      for ye in y_ends ]
  # line is returned as array of two points, [(x0,y0),(x1,y1)]
  blackest_lines.append([(img.ul.x, ys), (img.lr.x, \
    y_ends[ black_counts.index(min(black_counts)) ])])
